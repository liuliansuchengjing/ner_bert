...
def train(args, train_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            # 先在 CPU 上做 label 范围检查，避免 GPU 触发 device-side assert 后难排查
            labels_cpu = batch[3]
            bad = labels_cpu[(labels_cpu != -100) & ((labels_cpu < 0) | (labels_cpu >= len(args.label2id)))]
            if bad.numel() > 0:
                raise RuntimeError(
                    f"Found out-of-range labels: {bad.unique().tolist()} / num_labels={len(args.label2id)}"
                )
            ...
            # ====== 对抗训练：同样用同一套 loss_fct 重新算一次 ======
            if args.do_adv:
                fgm.attack()
                with torch.cuda.amp.autocast(enabled=use_amp):
                    outputs_adv = model(**inputs)
                    logits_adv = outputs_adv[0]
                    loss_adv = loss_fct(logits_adv.view(-1, logits_adv.size(-1)), labels.view(-1))
                    if args.n_gpu > 1:
            ...
            pbar(step, {'loss': float(loss.item())})
            tr_loss += loss.item()
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(
            ...
            label_all_subtokens=False,  # ä½ çŽ°åœ¨çš„å®žçŽ°æ˜¯â€œåªæ ‡é¦– subtokenâ€
        )
